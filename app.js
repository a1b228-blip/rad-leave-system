...
(function () {
  ...
  // 預設全科預假規範說明文件 (2027 最新修訂版)
  const DEFAULT_RULES_DOC = `🏥 佳里奇美醫院 放射診斷科
2027 年度員工線上預假規範與操作須知（最新修訂版）

一、 系統登入與帳號資訊
1. 登入帳號：個人 員工編號（例如：961137、A105W2、A00534 等）。
2. 預設密碼：預設密碼即為您的 員工編號。
3. 萬能輸入機制：系統已啟用大小寫與前後空格自動校正，請同仁安心登入。

二、 全科每日預假名額上限與國定假日規定
1. 平常日（週一至週五）：每日預假名額上限為 2 人。
2. 禮拜六（週六）：每日預假名額上限為 1 人。
3. 禮拜天（週日）：原則上 不開放自主預假（0 人）。
4. 國定假日：
   - 依據《勞動基準法》第 37 條規定，元旦、小年夜、除夕、初一~初三、228 紀念日、兒童/清明節、5/1 勞動節、端午節、中秋節及國慶日，系統皆自動標示 🚩 節日 標籤。
   - 國定假日每日自主預假名額預設為 0 人（封鎖自主預假），如因特殊公務或輪值需求，須由主管評估後特許處理。

三、 假別選擇與年假時數扣減機制
同仁於線上搶預假時，系統會跳出假別選單，請依實際需求選擇：
1. 🏖️ 一般年假：
   - 預假成功後，系統將自動從同仁個人剩餘年假配額中 扣除 8 小時。
   - 若剩餘年假時數不足 8 小時，系統將自動封鎖無法發起一般年假預假。
2. 💼 公出／公假：
   - 因公務派兼、講習、受訓等事由預假時，請選擇「公出/公假」。
   - 時數不扣減：公出假單 扣除 0 小時（不扣除同仁個人的年假時數）。
   - 名額佔用：公出同等佔用當天全科預假人數名額，日曆已預假名冊中會醒目標示 💼公出 標籤。

四、 每月 15 號「下月份自主預假」自動鎖定規則
1. 15 號截止時間：
   - 每月 15 號（含）起，系統會自動關閉並鎖定同仁對「下一個月份」的線上自主預假功能（例如：8 月 15 號起，系統將自動關閉同仁對 9 月份的自主預假）。
2. 排班結算作業：
   - 15 號鎖定後，同仁端日曆會顯示黃色鎖定警告 Banner，按鈕轉為 🔒 15號起關閉 (須向主管申請)，以利主管進行全科次月班表結算與派班作業。

五、 15 號後緊急預假與「主管代預假／特許強行預假」
1. 緊急預假申請：
   - 若同仁於每月 15 號鎖定後，或預假當日名額已滿（如 2/2 人已額滿）時有緊急預假或調班需求，不可以自行強行搶假，必須直接向主管提出預假申請。
2. 主管特許權限與額滿警示：
   - 主管具備最高排班管理權限，可於全年度任何時間點，在主管控制台使用「手動代同仁預假」模組幫同仁掛假（可選年假或公出）。
   - 若預假當日名額已滿，主管點擊掛假時系統會跳出 「此日預假已額滿，是否確定強行預假？」 警示視窗，經主管確認後即可完成特許強行預假。

六、 主管同仁名冊排序與「月預假表 Excel」匯出
1. 名冊手動拖拉排序：
   - 主管可於控制台名冊表格最左側，透過 ⣿ 拖拉把手自訂同仁名冊的上下順序，全系統選單與報表將 100% 同步連動。
2. 匯出月預假表 Excel (.xlsx)：
   - 主管點擊 🟢 匯出月預假表 Excel 按鈕，可直接產出帶有當下選取年月（例如：佳里奇美醫院_放射診斷科_2027年01月_工作人員月預假表.xlsx）的橫向 1:1 派班矩陣 Excel 檔案，並儲存至本地端。

七、 附則與注意事項
1. 本規範經放射診斷科主管核定後公布實施，修訂時亦同。
2. 請全科同仁共同遵守預假時間與名額規定，以維護臨床放射診斷業務之正常運作。
